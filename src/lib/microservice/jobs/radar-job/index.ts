...
// ─── Main service function — no session dependency ───────────────────────────
export async function runRadarJob(companyId: string) {
  const [company, brandEntity, geoDataSources, llmTopics, shopifyProducts, rivals] =
    await Promise.all([
      ...
      prisma.brandEntity.findUnique({
        where: { companyId },
        include: { sameAsLinks: true, offerings: true },
      }),
  ...
  const website  = company.website ?? geoDataSources.find((s) => s.label === "Website URL")?.rawContent?.trim() ?? ""
  ...
  const brandOfferings = brandEntity?.offerings.map((o) => ({
    product: o.name ?? undefined, productType: o.offeringType ?? undefined,
    url: o.url ?? undefined, differentiators: o.differentiators ?? [],
    useCases: o.useCases ?? [], targetAudiences: o.targetAudiences ?? [],
  ...
  const shopifyOfferings = shopifyProducts
    ...
    .map((p) => ({
      product: p.title.trim(), productType: "PRODUCT",
      url: p.onlineStoreUrl ?? undefined,
      differentiators: [] as string[], useCases: [] as string[],
      targetAudiences: [] as string[], competitorGroups: [] as string[],
    }))
  ...
  return { input, topics: radarOutput.topics, prompts: radarOutput.prompts,
           citations: radarOutput.citations, metrics: normalizedMetrics }
}