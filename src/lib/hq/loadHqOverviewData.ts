...
import { loadGeoKnightTopicViewsForCompanies } from "@/lib/geo/geoknight/loadGeoKnightTopicViews";
import { aggregateCitationContextFromIntel } from "@/lib/geo/radar/aggregateCitationContext";
import { buildDailySparkSeries } from "@/app/(Pages)/(WorkSpace)/geo/report/spark-data";
import { pickHighlightPrompts } from "@/app/(Pages)/(WorkSpace)/geo/report/pick-highlight-prompts";
...
export async function loadHqOverviewData(companyIds: string[], organizationName: string) {
  const ids = [...new Set(companyIds.map((id) => id.trim()).filter(Boolean))];
  if (ids.length === 0) {
    throw new Error("loadHqOverviewData: at least one company id required");
  }
  const cw =
    ids.length === 1 ? { companyId: ids[0]! } : { companyId: { in: ids } };

  const [payload, geoKnight, rivals, ownCompanyCitations] = await Promise.all([
    buildRadarGetPayload(prisma, ids),
    loadGeoKnightTopicViewsForCompanies(ids, organizationName),
  ...
  const perCompanyRadar =
    ids.length <= 1
      ? []
      : await Promise.all(
          ids.map(async (id) => {
            const p = await buildRadarGetPayload(prisma, [id]);
            return {
              companyId: id,
              companyName: p.company?.name?.trim() || id,
              sovSeries: p.sovSeries,
              modelBreakdown: p.modelBreakdown,
            };
          })
        );
  ...
  const recentCitations = ownCompanyCitations.map((c) => {
    ...
    return {
      ...
      companies: siblings,
      companyName:
        ids.length > 1 ? (c.company?.name?.trim() ?? null) : undefined,
    };
  });
